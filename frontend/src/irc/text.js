...
const applyTokenizer = tokenizerFn =>
  fp.pipe([
    fp.map(token => (token.type === 'text' ? tokenizerFn(token) : token)),
    fp.flatten,
  ])

const tokenize = ({ highlights } = {}) =>
  fp.pipe([
    ...
    (prev, token) => {
      switch (token.type) {
        ...
        case 'highlight':
          ...
          const styles = fp.reject(
            fp.isUndefined,
            fp.map(k => (prev[k] ? k : undefined), [
              'bold',
              'italic',
              'underline',
            ]),
