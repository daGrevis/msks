...
describe('lexer', () => {
  ...
  it('tokenizes text', () => {
    expect(tokenize()(s)).toEqual([createToken('text', s)])
  })

  it('tokenizes links', () => {
    expect(tokenize()('http://dagrev.is')).toEqual([
      createToken('link', 'http://dagrev.is'),
    ])

    expect(tokenize()('http://dagrev.is & http://dagrevis.lv')).toEqual([
      createToken('link', 'http://dagrev.is'),
      createToken('text', ' & '),
  ...
  it('tokenizes colors', () => {
    expect(tokenize()(colorize([4], s))).toEqual([
      createToken('foreground', 4),
      createToken('text', s),
    ])

    expect(tokenize()(colorize([4, 1], s) + ' ' + colorize([2], s))).toEqual([
      createToken('foreground', 4),
  ...
  it('tokenizes resets', () => {
    expect(tokenize()(s + RESET_CHAR)).toEqual([
      createToken('text', s),
      createToken('reset'),
    ])

    expect(tokenize()(colorize([4], s) + RESET_CHAR + ' world')).toEqual([
      createToken('foreground', 4),
      createToken('text', s),
      createToken('reset'),
  ...
  it('tokenizes styles', () => {
    expect(tokenize()(bold(s))).toEqual([
      createToken('bold'),
      createToken('text', s),
      createToken('bold'),
    ])
...
describe('parser', () => {
  ...
  it('parses italic', () => {
    expect(parse()(italic('hello'))).toEqual([
      { text: 'hello', styles: ['italic'] },
    ])
  })

  it('parses underline', () => {
    expect(parse()(underline('hello'))).toEqual([
      { text: 'hello', styles: ['underline'] },
    ])
  })

  it('parses bold/italic', () => {
    expect(parse()(bold(italic('hello')))).toEqual([
      { text: 'hello', styles: ['bold', 'italic'] },
    ])
  })

  it('parses bold/italic & underline', () => {
    expect(parse()(`${bold(italic('hello'))} ${underline('world')}`)).toEqual([
      { text: 'hello', styles: ['bold', 'italic'] },
      { text: ' ' },
  ...
  it('parses bold & more bold', () => {
    expect(parse()(`${bold('bold')} & ${bold('more bold')}`)).toEqual([
  ...
  it('parses foreground', () => {
    expect(parse()(colorize([4], 'hello'))).toEqual([
      { text: 'hello', foreground: 4 },
    ])
  })

  it('parses foreground & background', () => {
    expect(parse()(colorize([4, 7], 'hello'))).toEqual([
      { text: 'hello', foreground: 4, background: 7 },
    ])
  })

  it('parses rgb', () => {
    const s = colorize([4], 'r') + colorize([3], 'g') + colorize([2], 'b')
  ...
  it('parses foreground & bold', () => {
    expect(parse()(colorize([4], bold('hello')))).toEqual([
      { text: 'hello', foreground: 4, styles: ['bold'] },
    ])
  })
  ...
  it('remembers previous background', () => {
    const s = colorize([3, 1], 'x') + colorize([4], 'mas')
    expect(parse()(s)).toEqual([
  ...
  it('parses highlight', () => {
    ...
    ).toEqual([{ text: 'foo', isHighlight: true }])
  })
})
